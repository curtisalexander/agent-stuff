...
const result = await withPage({ createNew: !reuseTab, navigateTo: url, timeout }, async ({ client, sessionId }) => {
	...
	const expression = [
		...
		"for (const node of document.querySelectorAll('script, style, noscript, svg, canvas, iframe')) node.remove();",
		"let root = null;",
		...
		"const rawText = String((root && (root.innerText || root.textContent)) || '');",
